...
- **blockers.md** - (オプション) ブロッカーの記録
  - 作業中に発生した問題・障害の記録
  - 記録項目：発生日時、問題内容、原因（判明している場合）、対処策・解決策、ステータス（未解決／解決済）
  - 作業の中断・遅延が生じた際に作成する

- **decisions.md** - (オプション) 重要な決定事項の記録
  - 実装中に下した重要な設計判断・技術選択の記録
  - 記録項目：決定日、背景・コンテキスト、検討した選択肢、決定内容、採用理由・根拠
...
オプションファイルは作業中に必要が生じた時点で作成します：
- 問題・障害が発生した場合 → `blockers.md` を作成
- 重要な設計判断・技術選択が生じた場合 → `decisions.md` を作成
